...
pindx_t sigProceso ( void ) {                  /* planificador (scheduler) */
...
int activarProceso ( pindx_t pindx ) {
/*
if (debugWord == 0x0001) {
  printStrBIOS("\n activarProceso(): ") ;
  printStrBIOS("\n indProcesoActual = ") ;
...
void salvarProceso ( void ) {                    /* indProcesoActual != -1 */
  descProceso[indProcesoActual].trama =
    (trama_t *)pointer(SS_Proceso, SP_Proceso) ;
}
...
/*  rec_hijo = crearRec("HIJO", rOtro, sinInt) ;  */
  ...
  ccbAlEpilogo = (ccb_t)pointer(DS_SO1H, (word_t)&descCcbAlEpilogo) ;
  ...
  for ( i = maxProcesos-1 ; i > 0 ; i-- ) {
    ...
    descProceso[i].trama = (trama_t *)NULL ;
  ...
  descProceso[0].trama = (trama_t *)pointer(DS_SO1H, cabecera->SP0-sizeof(trama_t)) ;
