...
#include <so1h.h\procs.h>                              /* indProcesoActual */
...
pid_t kk_fork ( void )
{
    ...
    origen = MK_P(descProceso[indProcesoActual].CSProc, 0x0000) ;
    destino = MK_P(segmento, 0x0000) ;
//	copiaLarga(origen, destino, (dword_t)16*(dword_t)tam) ;
    memcpy(destino, origen, (dword_t)16*(dword_t)tam) ;
    ...
    SS_NuevoProceso = segmento
                      + (seg((pointer_t)descThread[indProcesoActual].trama)
                         - descProceso[indProcesoActual].CSProc) ;

    descThread[pindx].trama = (trama_t *) MK_FP(
                                  SS_NuevoProceso,
                                  off((pointer_t)descThread[indProcesoActual].trama)
                              ) ;
    descProceso[pindx].CSProc = segmento ;

    //descThread[pindx].trama->DS = SS_NuevoThread ;
    //descThread[pindx].trama->ES = SS_NuevoThread ;

//	descThread[pindx].trama->DI = descThread[indProcesoActual].trama->DI ;
//	descThread[pindx].trama->SI = descThread[indProcesoActual].trama->SI ;
//	descThread[pindx].trama->BP = descThread[indProcesoActual].trama->BP ;
//	descThread[pindx].trama->SP = descThread[indProcesoActual].trama->SP ;
//	descThread[pindx].trama->BX = descThread[indProcesoActual].trama->BX ;
//	descThread[pindx].trama->DX = descThread[indProcesoActual].trama->DX ;
//	descThread[pindx].trama->CX = descThread[indProcesoActual].trama->CX ;
//	descThread[pindx].trama->AX = descThread[indProcesoActual].trama->AX ;
//	descThread[pindx].trama->IP = descThread[indProcesoActual].trama->IP ;
    descThread[pindx].trama->CS = segmento ;
//	descThread[pindx].trama->Flags = descProceso[indProcesoActual].trama->Flags ;
