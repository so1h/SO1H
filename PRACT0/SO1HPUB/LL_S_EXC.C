...
#include <so1pub.h\ll_s_exc.h>                                /* nVIntSO1H */
...
pid_t createProcess ( const char * nombre,                   /* ax = 0000h */
                      const char * comando ) {
...
int exec ( const char * nombre,                              /* ax = 0002h */
           const char * comando ) {
  word_t err ;
  asm {
    les bx,nombre ;                                       /* ES:BX         */
...
pid_t waitpid ( pid_t pid, int * statloc ) {                 /* ax = 0003h */
  pid_t pidDifunto ;
  int status ;
