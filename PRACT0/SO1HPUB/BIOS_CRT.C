...
#include <so1hpub.h\bios_0.h>                    /* readXYBIOS, goToXYBIOS */
#include <so1hpub.h\bios_crt.h>                         /* tipoAdaptador_t */
...
void inicBiosCrt ( void )
{
    ...
    ptrPant = (pantalla_t *)MK_P_SEG(segMemVideo) ;
...
///////////  redimensionar(50, 80) ; /* da problemas con msdos de Takeda */
...
fontSize_t getFontSize ( void )
{
    ...
    case _MDA :
        res = 10 ;
        break ;  /* POINTS is not certain on MDA/Hercules */
    case _EGACOLOR :
    case _EGAMONO  :
        switch (res)
        {
...
tipoAdaptador_t tipoDeAdaptador ( void )    /* Determina el tipo de adapt. */
{
    ...
    (
        ...
        "   int 10h        \n"
        "   mov [bp-12],bx \n" /* DCC */       /* BL = ID del display activo   */
        /* BH = ID del display inactivo */
        "   mov [bp-16],al \n" /* reg_AL */
    ) ;

    if (reg_AL == 0x1a)           /* AL == 0x1a si la función esta soportada */
    {
    ...
    else
    {
        ...
        if ((reg_BL != 0x10) && (res == _NONE))       /* Hay un BIOS EGA */
        {
            ...
            if (reg_BH == 0x00)      /* configuracion del BIOS por defecto */
                res = _EGACOLOR;
            else
                res = _EGAMONO;
        }
    ...
    */
    ...
    printStrBIOS("DCC = 0x") ;
    printHexBIOS(DCC, 4) ;
    printStrBIOS(" CRTC = 0x") ;
    printHexBIOS(CRTC_base, 4) ;
    printStrBIOS(" segmento = 0x") ;
    printHexBIOS(segMemVideo, 4) ;
#else
...
void set25x80 ( void )
{
    ...
    case _MCGACOLOR :
    case _MCGAMONO  :
        establecerModoDeVideo(0x03) ;
        break ;
    case _MDA       :
        establecerModoDeVideo(0x07) ;
        break ;
    }
}
