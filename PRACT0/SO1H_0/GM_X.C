...
#include <so1hpub.h\def_mem.h>                           /* tamParagrafo_t */
#include <so1h_0.h\ajust_0.h>                                   /* modoSO1 */
#include <so1h_0.h\so1h_0.h>                       /* dirInicial, dirFinal */
#include <so1h_0.h\procs_x.h>           /* ptrListaLibres, ptrTamBloqueMax */
#include <so1h_0.h\gm_x.h>   
...
void inicGM_x ( void )                           /* 1 paragrafo = 16 bytes */
{
    ...
    primerSegLibre = (dirFinal + (tamParagrafo-1)) >> 4 ;
    printStrVideo("\n\n primerSegLibre = ") ;
...
//  /* bloque cabecera de la lista doblemente enlazada */
    ...
    *ptrListaLibres = (ptrBloque_t)MK_P_SEG(ptrBloque->sig) ;
    (*ptrListaLibres)->tam = 1 ;                               /* 16 Bytes */
    (*ptrListaLibres)->sig = SEG(ptrBloque) ;
    (*ptrListaLibres)->ant = (*ptrListaLibres)->sig ;

    *ptrTamBloqueMax = ptrBloque->tam ;
...
void inicGM_xOld ( void )                        /* 1 paragrafo = 16 bytes */
{
